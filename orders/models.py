...
class ProductInOrder(models.Model):
	order = models.ForeignKey(Order, on_delete=models.CASCADE, blank = False, null = True, default = None,  related_name ='order')
	customer = models.ForeignKey(Customer, on_delete=models.CASCADE, blank = True, null = True, default = None,  related_name = 'customer_pi')
...
class Basket(models.Model):
	customer = models.ForeignKey(Customer, on_delete=models.CASCADE, default = None,  related_name = 'customer_basket')
	created = models.DateTimeField(auto_now_add = True, auto_now = False)
	updated = models.DateTimeField(auto_now_add = False, auto_now = True)
	def __str__(self):
		return "Заказ %s статус %s" % (self.id, self.created)
	class Meta:
		ordering = ['-created']
...
class ProductInBasket(models.Model):
	basket = models.ForeignKey(Basket, on_delete=models.CASCADE, blank = False, null = True, default = None,  related_name ='basket')
	customer = models.ForeignKey(Customer, on_delete=models.CASCADE, blank = True, null = True, default = None,  related_name = 'customer_productinbasket')
	product = models.ForeignKey(Product, on_delete= models.CASCADE, blank = False, null = True, default = None,  related_name ='productinbasket')
	number = models.IntegerField(default = 0)
	is_active = models.BooleanField(default=True)
	price_per_item = models.PositiveIntegerField(default = 1)
	total_price = models.DecimalField(max_digits = 10, decimal_places = 2, default = 0) #price * number
